...
#         df = df.copy()
#         for attr in self.categorical_attributes_used:
#             temp_df = self.__get_top_values_df(
#                 df, attr, self.top_cat_dict).astype('category')
...
class DataTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, top=10, dummy_encode=True):
        ...
        self.all_cat_columns_in_raw_data_hardcoded = ['Airline',
                                                      'Destination',
                                                      'AircraftType',  # remember to change all rows to string outside
                                                      'FlightType',
                                                      'Sector']
